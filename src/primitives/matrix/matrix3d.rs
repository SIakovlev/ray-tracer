...
impl Matrix3D {
	...
	pub fn submatrix(&self, row_idx_skip: usize, col_idx_skip: usize) -> Matrix2D {
		let mut tmp: [[f64; MATRIX_SIZE - 1]; MATRIX_SIZE - 1] =
			[[0.0; MATRIX_SIZE - 1]; MATRIX_SIZE - 1];
...
mod tests {
	...
	#[test]
	fn indexing() {
		let m = Matrix3D::new([[-3.0, 5.0, 0.0], [1.0, -2.0, -7.0], [0.0, 1.0, 1.0]]);
	...
	#[test]
	fn equality() {
		let m1 = Matrix3D::new([[-3.0, 5.0, 0.0], [1.0, -2.0, -7.0], [0.0, 1.0, 1.0]]);

		let m2 = Matrix3D::new([[-3.0, 5.0, 0.0], [1.0, -2.0, -7.0], [0.0, 1.0, 1.0]]);

		assert_eq!(m1, m2);

		let m3 = Matrix3D::new([[-3.0, 5.0, 0.0], [1.0, -2.0, -7.0], [0.0, 1.0, -1.0]]);
	...
	fn submatrix() {
		...
		let m1 = Matrix3D::new([[1.0, 5.0, 0.0], [-3.0, 2.0, 7.0], [0.0, 6.0, -3.0]]);

		let m2 = m1.submatrix(0, 2);

		let m = Matrix2D::new([[-3.0, 2.0], [0.0, 6.0]]);
	...
	#[test]
	fn minor() {
		let m1 = Matrix3D::new([[3.0, 5.0, 0.0], [2.0, -1.0, -7.0], [6.0, -1.0, 5.0]]);
	...
	fn cofactor() {
		...
		let m1 = Matrix3D::new([[1.0, 2.0, 6.0], [-5.0, 8.0, -4.0], [2.0, 6.0, 4.0]]);
