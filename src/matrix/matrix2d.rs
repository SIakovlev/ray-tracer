...
mod tests {
	...
	#[test]
	fn indexing() {
		let m = Matrix2D::new([[-3.0, 5.0], [1.0, -2.0]]);
		assert_eq!(m[(0, 0)], -3.0);
		assert_eq!(m[(1, 0)], 1.0);
		assert_eq!(m[(0, 1)], 5.0);
		assert_eq!(m[(1, 1)], -2.0);
	...
	#[test]
	fn equality() {
		let m1 = Matrix2D::new([[-3.0, 5.0], [1.0, -2.0]]);

		let m2 = Matrix2D::new([[-3.0, 5.0], [1.0, -2.0]]);

		assert_eq!(m1, m2);

		let m3 = Matrix2D::new([[3.0, 5.0], [1.0, 2.0]]);
	...
	fn multiply() {
		...
		let m2 = Matrix2D::new([[-1.0, 2.0], [1.0, -2.0]]);

		let m3 = Matrix2D::new([[8.0, -16.0], [-3.0, 6.0]]);
